...
var soundJump = new Audio("img/wing.ogg");
var soundScore = new Audio("img/point.ogg");
var soundHit = new Audio("img/hit.ogg");
var soundDie = new Audio("img/die.ogg");
var soundSwoosh = new Audio("img/swooshing.ogg");
...
var FB = {
  ...
  init: function() {
    ...
    FB.ios = (FB.ua.indexOf('iphone') > -1 || FB.ua.indexOf('ipad') > -1) ?
      true : false;
...
FB.Draw = {
  ...
  Sprite: function(img, srcX, srcY, srcW, srcH, destX, destY, destW, destH, r) {
    ...
    FB.ctx.drawImage(img, srcX, srcY, srcW, srcH, destX, destY, destW,
      destH);
    FB.ctx.restore();
  },
  ...
  text: function(string, x, y, size, col) {
    FB.ctx.font = 'bold ' + size + 'px Fira Sans';
    FB.ctx.fillStyle = col;
...
FB.BottomBar = function(x, y, r) {
  ...
  this.render = function() {
    FB.Draw.rect(this.x, this.y, this.r, 100, '#333');
    for (var i = 0; i < 10; i++) {
      // FB.Draw.semiCircle(this.x + i * (this.r / 9), this.y, 20, '#444');
    }
  };
...
FB.Tree = function(x, y) {
  ...
  this.render = function() {
    ...
    FB.Draw.circle(this.x + this.r, (this.y + this.r) - 10, this.r, '#000',
      '#000');
    FB.Draw.circle(this.x + (this.r / 2), (this.y + this.r) - 10, this.r /
      3, 'rgba(0,0,0,0.0)');
    FB.Draw.rect(this.x + this.r, this.y + this.r, 10, this.r, '#000',
      '#d20');
  };
...
FB.Pipe = function(x, w) {
  ...
  this.render = function() {
    if (this.coin) {
      FB.Draw.circle(this.centerX + this.w / 2 - 5, this.centerY - 5, 5, "#ff4302");
    }
    var topfork = new Image();
    var bottomfork = new Image();
    topfork.src = "img/fork-top.png";
    bottomfork.src = "img/fork-bottom.png";
    // FB.Draw.rect(this.centerX, 0, this.w, this.centerY - 70, '#111fff');
    // FB.Draw.rect(this.centerX, this.centerY + 70, this.w, this.h - this.centerY, '#111fff');
    FB.Draw.Image(topfork, this.centerX - 5, this.centerY - 236 - 60);
    FB.Draw.Image(bottomfork, this.centerX - 5, this.centerY + 60);
  };

  this.respawn = function() {
    this.centerY = this.randomIntFromInterval(120, 300);
    // this.centerX = 300 - this.w + FB.WIDTH * 0.8;
    this.centerX = 600;
    this.coin = true;
  };
...
FB.Bird = function() {
  this.img = new Image();
  this.img.src = 'img/tomato_50.png';
  this.gravity = 0.15;
  this.width = 50;
  this.height = 47.5;
  this.ix = 0;
  ...
  this.render = function() {
    FB.Draw.Sprite(this.img, this.ix, this.iy, this.width, this.height,
      this.vx, this.vy, this.width, this.height, this.rotation);
  };
...
window.Splash = function() {
  ...
  this.banner.src = "img/splash.svg";
  ...
  this.init = function() {
    ...
    // FB.entities.push(new FB.Tree(~~(Math.random() * FB.WIDTH), FB.HEIGHT -
    //   160));
    // FB.entities.push(new FB.Tree(~~(Math.random() * FB.WIDTH + 50), FB.HEIGHT -
    //   160));
    // FB.entities.push(new FB.Tree(~~(Math.random() * FB.WIDTH + 100), FB.HEIGHT -
      // 160));
  };
...
window.Play = function() {
  ...
  this.init = function() {
    var scale = 0.6;
    FB.entities.push(new FB.Pipe(FB.WIDTH * 2 * scale, 10));
    FB.entities.push(new FB.Pipe(FB.WIDTH * 2.5 * scale, 10));
    FB.entities.push(new FB.Pipe(FB.WIDTH * 3 * scale, 10));
    FB.entities.push(new FB.Pipe(FB.WIDTH * 3.5 * scale, 10));
    FB.entities.push(new FB.Pipe(FB.WIDTH * 4 * scale, 10));

    FB.bird = new FB.Bird();
    FB.entities.push(FB.bird);
    for (var n = 0; n < 10; n++) {
      var img = new Image();
      img.src = "img/font_small_" + n + '.png';
      FB.fonts.push(img);
    }
    FB.digits = ["0"];
  };
  ...
  this.render = function() {
    //score
    FB.Draw.text(FB.score.coins, (FB.WIDTH / 2) - 13, 60, 40, 'white');
    ...
    //   Image(FB.fonts[Number(FB.digits[i])], X + (i * 14), 10);
    // }
  };
...
window.GameOver = function() {
  ...
  this.init = function() {
    ...
    setTimeout(function() {
      ...
      that.banner.src = "img/scoreboard.svg";
      var m = that.getMedal();
      // that.medal = new Image();
      // that.medal.src = 'img/medal_' + m + '.png';
      that.replay = new Image();
      that.replay.src = "img/replay.svg";
  ...
  this.render = function() {
    if (this.banner) {
      FB.Draw.Image(this.banner, 65, 70);
      FB.Draw.Image(this.replay, 65, 260);
      ...
      FB.Draw.text(FB.score.coins, (FB.WIDTH / 2) - 13, 260, 40, 'white');
    }
  };
